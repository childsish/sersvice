...
worker.onmessage = function(e) {
    ...
    if (data.message == 'result') {
        $('#content').append('<tr><td>' + data.content[0] + '</td><td>' + data.content[1] + '</td><td>' + data.content[2] + '</td><td>' + data.content[3] + '</td><td>' + data.content[4] + '</td><td>' + data.content[5] + '</td></tr>')
    }
...
$('#run').click(function() {
    ...
    $('#content').html('<tr><th>Chromosome</th><th>Position</th><th>Genomic variant</th><th>Gene name</th><th>Coding variant</th><<th>Amino acid variant</th></tr>')
    worker.postMessage( {
        message: 'run'
    })
})
